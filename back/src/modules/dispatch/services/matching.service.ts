...
export class MatchingService {
  ...
  async solve(batch: PoolBatch): Promise<MatchingResult> {
    ...
    const { costMatrix, candidateMatrix, distances } = this.buildCostMatrix(batch, availableDrivers)
  ...
  private buildCostMatrix(batch: PoolBatch, drivers: DriverCandidate[]) {
    ...
    for (let i = 0; i < trips.length; i++) {
      ...
      for (let j = 0; j < drivers.length; j++) {
        ...
        const h3Distance = this.safeGridDistance(trip.pickup.h3Index, driver.location.h3Index)
